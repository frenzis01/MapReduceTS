...
{As we have discussed MapReduce during the lectures, it includes the steps depicted in Fig. \ref{fig:mapreduce}, i.e.:\ns
\begin{enumerate}
...
\begin{enumerate}
   ...
   \note{Clearly the project includes basic implementations of the \texttt{source} and \texttt{sink nodes}. }
\end{enumerate}
...
\framedt{CAP theorem recalls}{
   % // TODO verify this 
   CAP theorem states that we can have at the same time only two properties among \textit{Consistency}, \textit{Availability} and \textit{Network Partitioning}. In fact in our network-partitioned scenario, enforcing consistency leads to periods of unavailability, while guaranteeing (better) availability implies the possibility of ending up in an uncosistent state, with some information lost.
}
...
% Riassumiamo il comportamento dell'infrastruttura sviluppata.