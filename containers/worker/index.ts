   ...
   pipelinesConsumer.run({
      eachMessage: async ({ message }: { message: KafkaMessage }) => {
         try {
            ...
            // When map has started it automatically subscribes to new pipelines
            ...
            //    console.log(`Subscribing ${MAP_TOPIC}`);
            ...
            // } else
             if (MODE === '--reduce') {
               ...
               console.log(`Subscribing ${REDUCE_TOPIC}`);
            ...
            //    console.log(`Subscribing ${SHUFFLE_TOPIC}`);
            ...
            if (error instanceof Error) {
               ...
               if (error.stack) {
                  console.error(`[Pipeline Update/ERROR] [${GROUP_ID}/${WORKER_ID}] ${error.stack}`);
               }
            } else {
   ...
   await consumer.run({
      ...
      eachMessage: async ({ topic, message }: { topic: any, message: KafkaMessage }) => {
         ...
         try {
            ...
            if (error instanceof Error) {
               console.error(`[MAP MODE] [${GROUP_ID}/${WORKER_ID}] ${error.message}`);
               if (error.stack) {
                  console.error(`[MAP MODE] [${GROUP_ID}/${WORKER_ID}] ${error.stack}`);
               }
            } else {
               console.error(`[MAP MODE] [${GROUP_ID}/${WORKER_ID}] Unknown error`, error);
            }
         }
      },
   });
...
async function processMessageMap(message: KafkaMessage, pipelineConfig: PipelineConfig) {
   ...
   if (isStreamEnded(message)) {
      ...
      if (!counter || Number(counter) !== BUCKET_SIZE) {
         ...
         console.log(`[MAP MODE] Received stream ended message. Got ${counter}/${BUCKET_SIZE} messages... for ${pipelineID}-MAP-ENDED-counter`);
         return;
      }
      else {
   ...
   for (const result of mapResults) {
      ...
      await producer.send({
         ...
         messages: [{ key: pipelineConfig.keySelector(result), value: JSON.stringify(newMessageValue(result, pipelineID)) }],
      });
   }
}
      ...
      eachMessage: async ({ message }: { message: KafkaMessage }) => {
         ...
         if (isStreamEnded(message)) {
            ...
            if (!streamEndedCounter || Number(streamEndedCounter) < BUCKET_SIZE) {
               console.log(`[SHUFFLE MODE] Not yet reached cap`);
               return;
            }
